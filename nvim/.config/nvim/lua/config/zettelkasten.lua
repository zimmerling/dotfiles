...
local function create_and_open_zk_note()
  ...
  if title then
    ...
    if file_path then
      ...
      vim.notify("Created and opened new note: " .. title)
      vim.notify("File path: " .. file_path) -- Debug print
    else
      vim.notify("Failed to create note: " .. title)
      vim.notify("Command output: " .. output)
    end
  else
    vim.notify("No title found between double square brackets")
  end
end
...
local function yank_and_search_markdown_link()
  ...
  if yanked_text ~= "" then
    ...
    vim.notify("No text found inside brackets")
  end
end
...
-- Add a startup test to confirm loading
vim.api.nvim_create_autocmd("VimEnter", {
  callback = function()
    vim.notify("🌟 Zk Configuration Startup Complete!", vim.log.levels.INFO)
  end,
})
